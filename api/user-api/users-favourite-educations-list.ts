...
function getUsersFavouriteEducationsList() {
  ipcMain.handle(
    'get-users-favourite-educations-list',
    async (_event, userId: number, params: PaginationParams = {}): ApiResponseProps<PaginatedData<UserEducationFavoriteListItem>> => {
      try {
        const db = getDb();
        ...
        const paginatedResponse = buildPaginatedResponse(favoritesList, total, page, limit);
